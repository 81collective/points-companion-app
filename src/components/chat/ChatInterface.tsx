...
import type { CardRecommendation } from '@/types/location.types';
...
import type { BestCardForBusiness } from '@/hooks/useBestCardsForBusinesses';
...
export default function ChatInterface({ mode, isAuthenticated: _isAuthenticated, userCards: _userCards, persistUiState = true, onModeChange, onPlanningRecommendations }: ChatInterfaceProps) {
  ...
  const perBusinessData = React.useMemo(() => {
    ...
    const filtered = hasWallet && walletOnly
      ? perBusinessBest.filter((item: BestCardForBusiness) => isOwnedCard(item.recommendation?.card.card_name))
      : perBusinessBest;
    const anyOwned = perBusinessBest.some((item: BestCardForBusiness) => isOwnedCard(item.recommendation?.card.card_name));
  ...
  // Wallet toggle handler removed as the UI enforces wallet-only behavior when available
            ...
            {permissionState.granted && !nearbyLoading && nearbyBusinesses && nearbyBusinesses.length > 0 && (
              <div className="mt-3">
                <ChatBubble
                  ...
                  richContent={(
                    <div className="mt-1 flex flex-col gap-2">
                              {(showAllNearby ? sortedNearby : sortedNearby.slice(0, 3)).map((b: Business) => (
                        <NearbyRow
                          key={b.id}
                          id={b.id}
                          name={b.name}
              ...
              <div className="mt-4">
                  ...
                  <div className="flex gap-3 pr-2">
                    {(
                      recommendationsData.hasWallet
                        ...
                        : (recommendations || [])
                    ).slice(0, 5).map((rec: CardRecommendation, idx: number) => (
                ...
                {recommendationsData.hasWallet && recommendationsData.display.length === 0 && (recommendations && recommendations.length > 0) && (
                  <div className="mt-3">
                    ...
                    <div className="overflow-x-auto">
                      <div className="flex gap-3 pr-2">
                        {recommendations.slice(0,3).map((rec: CardRecommendation, idx: number) => (
              ...
              <div className="mt-4">
                  ...
                  <div className="flex gap-3 pr-2">
                    {(
                      perBusinessData.hasWallet
                        ...
                        : perBusinessBest
                    ).map((item: BestCardForBusiness, idx: number) => (
                ...
                {perBusinessData.hasWallet && perBusinessData.filtered.length === 0 && perBusinessBest.length > 0 && (
                  <div className="mt-3">
                    ...
                    <div className="overflow-x-auto">
                      <div className="flex gap-3 pr-2">
                        {perBusinessBest.slice(0,3).map((item: BestCardForBusiness, idx: number) => (
...
// Explicit module marker to ensure TypeScript treats this file as a module
export {};
